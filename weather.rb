...
# 1. Complete the method below called location, that accepts a hash as an argument so that it returns the value of city from the weather hash. 
...
# 3. Using the methods above and string interpolation, write a method called "weather_report" that returns a string that describes the current weather - for example: "The weather in New York is Partly Cloudy. The temperature is 66.1 degrees Farenheit." HINT: the method should RETURN the string, not print it to the terminal.



#4. Write a method called "city_weather_info" that takes in the weather hash as an argument. 
# You'll want to use the each method to iterate over the hash and print out the key-value pairs. An example response would be: 
